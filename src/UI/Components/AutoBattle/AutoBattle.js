...
function _getNearbyMobs() {
	...
	EntityManager.forEach(function (e) {
		if (e.objecttype !== Entity.TYPE_MOB || e.action === e.ACTION.DIE || e.remove_tick !== 0) return;
...
function _getMobListForMap() {
	...
	for (let i = 0; i < naviMobs.length; i++) {
		...
		if (!byClass[classId]) {
			...
			const name = tableName !== 'Unknown' ? tableName : naviName ? naviName : 'Monster #' + classId;
	...
	visible.forEach(function (m) {
		if (!byClass[m.job]) {
			...
			const name = tableName !== 'Unknown' ? tableName : dispName ? dispName : 'Monster #' + m.job;
...
function _refreshList(root) {
	...
	if (classes.length === 0) {
		...
		classes.forEach(function (info) {
			...
			const countLabel = tot > 0 ? vis + ' / ' + tot : vis > 0 ? '~' + vis : '?';
			row.innerHTML =
				'<div class="ab-mob-id">#' +
				info.classId +
				'</div>' +
				'<div class="ab-mob-name">' +
				_esc(info.name) +
				'</div>' +
				'<div class="ab-mob-count">' +
				countLabel +
				'</div>' +
				'<div class="ab-mob-select" data-class="' +
				info.classId +
				'">Select</div>';
